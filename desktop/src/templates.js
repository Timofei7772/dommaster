...
const { replaceBookmarkPlaceholdersInXml } = require('./word-template-utils')

function copyMissingTemplateTree(sourceDir, targetDir, fsImpl = fs, pathImpl = path) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      copyMissingTemplateTree(sourcePath, targetPath, fsImpl, pathImpl)
      continue
    }
...
function ensureWritableTemplatesPath({
  sourceRoot,
  userTemplatesRoot,
  fsImpl = fs,
  pathImpl = path,
}) {
  if (!sourceRoot || !fsImpl.existsSync(sourceRoot)) {
    return sourceRoot
  }

  try {
    copyMissingTemplateTree(sourceRoot, userTemplatesRoot, fsImpl, pathImpl)
    return userTemplatesRoot
  } catch (error) {
    console.warn('Не удалось подготовить пользовательские шаблоны, используем bundled templates:', error?.message || error)
    return sourceRoot
  }
}
...
function resolveTemplatesPath({
  ...
  if (!appImpl || typeof appImpl.isPackaged !== 'boolean') {
    return pathImpl.join(moduleDir, '..', 'db')
  }

  const isDev = !appImpl.isPackaged
  if (isDev) {
    return pathImpl.join(moduleDir, '..', 'db')
  }

  const sourceRoot = pathImpl.join(resourcesPath, 'db')
  const userTemplatesRoot = pathImpl.join(appImpl.getPath('userData'), 'templates')

  return ensureWritableTemplatesPath({
    sourceRoot,
    userTemplatesRoot,
    fsImpl,
    pathImpl,
...
const getTemplatesPath = () => resolveTemplatesPath()
...
const findTemplateInfo = (templateId) => {
...
function buildSplitPlaceholderPattern(placeholder) {
  const chars = ('[' + placeholder + ']').split('')
  const pattern = chars.map((c, index) => {
...
function inspectWordTemplatePlaceholders(templateId, options = {}) {
  const templateInfo = findTemplateInfo(templateId)
  if (!templateInfo) {
  ...
  if (!exists) {
    return {
      ...
      exists: false,
      placeholders: [],
      missingFields: options.requiredFields || [],
  ...
  const countFieldMatches = (field) => {
    const matches = xml.match(buildSplitPlaceholderPattern(field))
    return matches ? matches.length : 0
  }

  const presentRequiredFields = requiredFields.filter((field) => {
    const count = countFieldMatches(field)
    fieldCounts[field] = count
    return count > 0
  })
  const missingFields = requiredFields.filter((field) => !presentRequiredFields.includes(field))
  const forbiddenFieldsPresent = forbiddenFields.filter((field) => {
    const count = countFieldMatches(field)
    fieldCounts[field] = count
    return count > 0
  })
  const simpleMatches = Array.from(new Set((xml.match(/\[[^\]]+\]/g) || []).map((match) => match.slice(1, -1))))

  return {
    templateId,
    templatePath,
    exists: true,
    placeholders: Array.from(new Set([...simpleMatches, ...presentRequiredFields])),
    missingFields,
    forbiddenFieldsPresent,
    fieldCounts,
  }
...
const generateFromWordTemplate = (templateId, data, outputPath) => {
  ...
  const bookmarkData = Object.fromEntries(
    Object.entries(data || {})
      ...
      .map(([key, value]) => [key.slice('@bookmark:'.length), value])
  )
  ...
  if (documentXml) {
    ...
    xmlContent = replaceBookmarkPlaceholdersInXml(xmlContent, bookmarkData)
    xmlContent = replacePlaceholdersInXml(xmlContent, placeholderData)
  ...
  headerFooterFiles.forEach(fileName => {
    ...
    if (file) {
      ...
      content = replaceBookmarkPlaceholdersInXml(content, bookmarkData)
      content = replacePlaceholdersInXml(content, placeholderData)
...
module.exports = {
  ...
  inspectWordTemplatePlaceholders,
  getTemplatesList,
